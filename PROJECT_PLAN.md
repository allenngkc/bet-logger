...
- **Single-leg straight bets get an EV even when the lone leg is unpriced** — for a *one-leg* bet the combined odds ARE that leg's odds, so `devig.with_combined_fallback(legs, combined)` fills the lone unpriced leg from the (pre-boost) combined price before the `all_legs_priced`/EV gate — a straight bet with a profit token is thus still de-vigged and EV-counted. This is scoped to **exactly one leg**: for 2+ legs the combined price is never split/duplicated across legs (that fabricates a fair prob), so an unpriced parlay still reports 0 EV. A single leg that already carries its own odds keeps them (its own price wins).
